...
segment hours_tens = {0, 9, CATHODE_HOURS_TENS, hours_tens_digit_masks, &minutes_ones, segment_incrementer_standard};
segment hours_ones = {0, 9, CATHODE_HOURS_ONES, digit_masks, &hours_tens, segment_incrementer_for_hours_ones};
segment minutes_tens = {0, 5, CATHODE_MINUTES_TENS, digit_masks, &hours_ones, segment_incrementer_standard};
segment minutes_ones = {0, 9, CATHODE_MINUTES_ONES, digit_masks, &minutes_tens, segment_incrementer_standard};
...
inline void segment_render()
{
    ...
    LED_SEG_LAT = seg->digit_masks[seg->value];
    seg = seg->next;
}
...
{
    if (seg->value == 3 && (seg->next->value == 2 || seg->next->value == 6))
    {
        seg->value = 0;
        seg->next->value = 0;
        return 1;
